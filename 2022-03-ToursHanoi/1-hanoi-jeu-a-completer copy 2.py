...
def search_next_move_random(state):
    start = -1
    # valeur par défaut puisque c'est la cible
    end = 3

    possibilities = list(get_combinaisons())
    finish = False
    found = False
    while not finish:
        ...
        del possibilities[i]
        found = is_valid(state, (start, end))
...
def hanoi(state, n,a=0,b=1,c=2):
    if (n > 0):
        state = successor(state=state,move=(a,c))
...
def search_next_move_ia(state, last_move):
    start = 0
    target = 2
    # valeur par défaut puisque c'est la cible
    end = target
        ...
        while not is_end(state) and not exit:
            ...
            if "manuel" in mode:
                ...
                if "random" in mode:
                    ...
                    move = search_next_move_ia(state=state)
                if move is None:

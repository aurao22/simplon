...
def load_word_file(file_name, verbose=False):
    """ Charge le fichier de mot et les ajoute dans la liste de mots
                ...
                """
    if verbose:
        print("Function load_word_file ", file_name, end="")
    f = open(file_name, "r")
    for ligne in f:
        WORDS.append(ligne.strip())
    # Fermeture du fichier
    f.close()
    if verbose:
        myList.logEnd("Function load_word_file " + file_name, verbose)
...
def get_a_word(verbose=False):
    selectedWord = np.random.randint(0, len(WORDS))
...
def play_mot_cache(verbose=False):
    """ Jeu qui consistera à retrouver un mot, l'ordinateur ayant écrit ses lettres par ordre alphabétique.
            Args:
                verbose (True or False): True pour mode debug
            Returns:
                None
            """
    print("Function play_mot_cache")
    selectedWord = get_a_word()
    nb_essais = 0
    maskWord = sortWordLetter(selectedWord, verbose)
    propose = ""
    while propose != selectedWord:
        propose = input(maskWord + " => Entrez l'anagramme :")
        nb_essais += 1
    print("Bravo ! Vous avez réussi en ", nb_essais, "essais")
    myList.logEnd("Function play_mot_cache ", True)


def exo_7_3(verbose=False):
    # Exercice 7.3
    ...
    selectedWord = get_a_word(verbose)
    selectedWord = "Il était une fois avec mes voyelles : aàâeéèêëiïîoôuùûy"
    selectedWord += " " + selectedWord.upper()
    updatedword = replaceVoyelleBy(selectedWord, replace_car="*", verbose=verbose)
    print("Origine    :", selectedWord)
    print("Mis à jour :", updatedword)
...
verbose = False
# Exercice 7.2
file_name="211108_liste_mots.txt"
load_word_file(file_name, verbose)
...
saisi = None
while saisi != "exit":
    saisi = input("\nSaisissez le numéro de l'exercice ou exit. 7.1 à 7.3 ou a à h :")
    start_time = time.time()
    moyenne = 0
    nb_match_words = 0
    if saisi == "7.1":
        # Sélection sans random.choice()
        indice_au_hasard = randint(0, len(WORDS) - 1)
        print(WORDS[indice_au_hasard])
    elif saisi == "7.2":
        play_mot_cache(verbose)
    elif saisi == "7.3":
        exo_7_3(verbose)
    elif saisi == "a":
        """ a) Calculez le pourcentage de mots français où la seule voyelle est le « e » (il peut y en avoir
            plusieurs dans le mot). Par exemple : exemple, telle, égrener. """
        for word in DICO["FR"]:
        ...
        """c) Affichez les mots de moins de 10 lettres ayant comme lettre centrale un « w ». 
        Le mot doit avoir un nombre impair de lettres. Par exemple : edelweiss, kiwis. """
        for word in DICO["FR"]:
            taille = len(word)
            if taille % 2:
                centre = int(taille/2)
